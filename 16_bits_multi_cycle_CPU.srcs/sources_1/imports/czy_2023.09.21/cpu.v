...
wire  en_pc_pulse,en_group_pulse,alu_in_sel,en_alu,en_data_mem_pulse,we_pulse,en_ram_out;
wire   ram_select; 
wire  [1:0]  pc_ctrl;
wire  [3:0]  reg_en;
wire  [3:0]  alu_func;
...
reg [31:0] counter;  // 32-bit counter for frequency division
reg clk_slow;           // New slow clock
parameter DIV_FACTOR = 10000000; // Frequency dividing factor, you need to set this value according to your needs
...
always @(posedge clk or negedge rst) 
    ...
    begin   
        clk_slow <= ~clk_slow;  // Flip the status of clk_slow
        counter <= 0;           // The counter returns to zero
    end
...
// On each slow clock rising edge, assign reg_group_mux_out [7:0] to led_flowing
always @(posedge clk or negedge rst) 
if (!rst)
    begin   
       led_flowing <= 0;
